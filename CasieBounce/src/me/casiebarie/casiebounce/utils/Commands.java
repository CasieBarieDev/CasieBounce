...
import me.casiebarie.casiebounce.sqlite.ResetData;
		...
		if(cmd.getName().equalsIgnoreCase("CB") && args.length >= 1) {
			if(sender.hasPermission("CB.admin")) {
				...
				case "RESETDATA":
					...
					case "ALL": rD.createMessage(sender, "ALL", null); break;
					default: msg.send(sender, "&4USAGE: &c/cb ResetData <RegionPlayer/Region/Player/All>");} return true;
				case "CONFIRM": rD.confirm(sender, (sender instanceof Player) ? false : true); break;
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("CB") && sender.hasPermission("CB.admin")) {
			if(args.length == 1) {
				...
				switch (args[1].toUpperCase()) {
				case "REGIONPLAYER": dataBase.addAll(db.getCompletions("REGION", null)); break;
