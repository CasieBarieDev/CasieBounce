			...
			if(sender.hasPermission("CB.admin")) {
				if(args.length == 1) {
					if(args[0].toUpperCase().equals("RELOADCONFIG")) {
						...
						sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6[&bCasieBounce&6] &aReloaded config."));
						getconfig(sender);
					} else if(args[0].toUpperCase().equals("INFO")) {
						if(sender instanceof Player) {info(sender);
						} else {sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThis command can only be executed by a player!"));}
					} else {sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4USAGE: &c/cb <reloadconfig/info>"));}
				} else {sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4USAGE: &c/cb <reloadconfig/info>"));}
			} else {sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cYou don't have permission to use this Command!"));}
		} return false;
	}
	
	public void getconfig(CommandSender sender) {
		regionNames = config.getStringList(".Regions");
		bounceForge = config.getDouble(".Bounceforge");
